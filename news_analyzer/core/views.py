...
DEFAULT_PROMPT = (
    "Оцени важность этой новости для всех аудиторов Сбера: "
    "от 1 (неважно) до 10 (критически важно). "
    "Учитывай влияние на репутацию, регуляторные риски, "
    "финансовые последствия и операционные риски."
)

DEFAULT_SUMMARY_PROMPT = "Ты — редактор новостного Telegram-канала Сбера. Напиши краткую, живую и информативную сводку новости для публикации в ТГ-канале. Формат: 2-4 предложения, без заголовка, без хештегов, без эмодзи. Пиши ёмко и по делу."
...
SCALE_PROMPTS = {
    1: "Оцени влияние только на конкретный отдел или процесс.",
    2: "Оцени влияние на отдельное подразделение или региональный филиал.",
    3: "Оцени влияние на целое бизнес-направление.",
    4: "Оцени влияние на компанию (Сбер) в целом.",
    5: "Оцени глобальное влияние на всю отрасль и макроэкономику."
}
...
def _new_job(file_path, rated_path, sheet, column, threshold, prompt, generate_image, add_link, summary_prompt, image_prompt):
    return {
        "status": "running",
        "stage": "Запуск...",
        "current": 0,
        ...
        for position in range(processed_rows, total_rows):
            ...
            if text.strip() and text.strip().lower() != "nan":
                rate_task = ApiQueueTask(
                    ...
                    func=lambda text=text, prompt=prompt: gc.rate_importance(text, prompt),
            ...
            if score is not None and score >= threshold:
                ...
                summarize_task = ApiQueueTask(
                    ...
                    func=lambda text=text, p=summary_prompt: gc.summarize_news(text, p),
                ...
                if summary and generate_image_flag:
                    ...
                    image_task = ApiQueueTask(
                        ...
                        func=lambda s=summary_result, p=image_prompt: gc.generate_image(s, p),
...
def start_analysis(request):
    ...
    settings_data = _load_settings()
    base_prompt = str(data.get("prompt", "")).strip() or settings_data.get("active_prompt_text", DEFAULT_PROMPT)
    scale = int(data.get("scale", 1))
    scale_text = SCALE_PROMPTS.get(scale, SCALE_PROMPTS[1])
...
def regenerate_text(request):
    ...
    task = ApiQueueTask(
        ...
        priority=True,
        func=lambda p=job.get("summary_prompt", DEFAULT_SUMMARY_PROMPT): gc.summarize_news(original_text, p),
...
def regenerate_image(request):
    ...
    task = ApiQueueTask(
        ...
        priority=True,
        func=lambda p=job.get("image_prompt", DEFAULT_IMAGE_PROMPT): gc.generate_image(summary, p),
