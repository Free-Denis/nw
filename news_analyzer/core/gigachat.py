...
class GigaChatClient:
    ...
    def rate_importance(self, text, prompt):
        messages = [
            {
                "role": "system", 
                "content": prompt
        ...
        resp = self.completions(messages, temperature=0)
    ...
    def summarize_news(self, text, prompt):
        messages = [
            {"role": "system", "content": prompt},
            {"role": "user", "content": f"Текст новости:\n{text[:8000]}"}
        ]
        resp = self.completions(messages, temperature=0.3)
        return resp["choices"][0]["message"]["content"].strip()

    def generate_image(self, summary_text, prompt):
        payload = {
