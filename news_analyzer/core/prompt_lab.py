...
def _gc_call(gc, system_prompt, user_prompt, temperature=0.1, model=None):
    """Single GigaChat completion call, returns parsed JSON."""
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": user_prompt},
    ]
    resp = gc.completions(messages, temperature=temperature, model=model)
...
def _gc_call_text(gc, system_prompt, user_prompt, temperature=0.1, model=None):
...
def run_analysis_step(job_id):
    ...
    try:
        ...
        for idx, example in enumerate(examples):
            ...
            task = ApiQueueTask(
                ...
                func=lambda t=example["text"], l=example["label"], tp=topics: _gc_call(
                    ...
                    _build_analyze_single_user(t, l, tp), model=model
                ),
            )
        ...
        task = ApiQueueTask(
            ...
            func=lambda: _gc_call(gc, CONSOLIDATE_SYSTEM, _build_consolidate_user(analyses, user_input), model=model),
        )
        API_QUEUE.submit(task)
        task.done.wait()
        ...
        if ask_questions:
            ...
            task = ApiQueueTask(
                name="lab-questions",
                func=lambda: _gc_call(gc, QUESTIONS_SYSTEM, _build_questions_user(consolidation, user_input), model=model),
            )
            API_QUEUE.submit(task)
            task.done.wait()
    ...
    try:
        ...
        if is_iteration and prev_prompt and prev_policy:
            ...
            task = ApiQueueTask(
                ...
                func=lambda: _gc_call(gc, POLICY_SYSTEM, _build_policy_user(consolidation, user_input, answers), model=model),
            )
            API_QUEUE.submit(task)
            task.done.wait()
        ...
        for idx, example in enumerate(examples):
            ...
            task = ApiQueueTask(
                name=f"lab-test-{idx}",
                func=lambda t=example["text"], p=prompt_text: _gc_call(gc, p, t, model=model),
            )
            API_QUEUE.submit(task)
            task.done.wait()
        ...
        task = ApiQueueTask(
            ...
            func=lambda: _gc_call(gc, EVALUATE_SYSTEM,
                                   _build_evaluate_user(prompt_text, test_results, examples, threshold, diagnostics), model=model),
        )
        API_QUEUE.submit(task)
        task.done.wait()
